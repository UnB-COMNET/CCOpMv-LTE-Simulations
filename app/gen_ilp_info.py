...
class MovementSimulationRunner:
    ...
    def run(self) -> None:
        ...
        ilpc.ilp_move_users(
            scen=scen,
            filename=self.run_config.ini_path,
            n_macros=self.ues_config.n_macros,
            n_ues_macro=self.ues_config.n_ues,
            ues_per_slice=self.ues_config.ues_per_slice,
            config_name=self.run_config.config_name,
        )
        snapshot_filename = genf.gen_movement_filename(
            config_name=self.run_config.config_name,
            seed_val=self.run_config.chosen_seed,
            snapshot=True,
        )
        ...
        command = (
            rf"opp_runall -j{self.run_config.cpu_num} ./proj_make -f {self.run_config.ini_path} -u Cmdenv -c {self.run_config.config_name}"
            + rf" -n .:{inet_dir}/src:{inet_dir}/examples:{inet_dir}/tutorials:{inet_dir}/showcases:{simu5g_dir}/simulations:{simu5g_dir}/src"
        )
        arg = (
            f"cd {project_dir}\n"
            f'{venv_python_path} -m opp_env run -c "{command}"'
        )
...
class SolverRunner:
    ...
    def _run_solution(self) -> None:
        ...
        size_sector = scen.size_config.size_sector
        ...
        if get_solution:
            sinr_map = scen.getSinrMap()
            print(
                "Running Solver - Min Snr: {} - {} (Seed: {})".format(
            ...
            antennas_map_m = [
                (0 if random() < disaster_percentage / 100 else 1)
                for _ in range(scen.n_sectors)
            ]
            ...
            first_antenna_region = self._resolve_first_antenna(
                antennas_map_m, first_antenna_region, chosen_seed
            )
            print("-------------Generating ues map")
            users_t_m = get_map_ues_time(
                scen=scen, xml_filename=xml_filename, ues_per_slice=ues_per_slice
            )
            out_file = open(genf.gen_log_file_name(result_dir, file_name), "wb", 0)
            sys.stdout = io.TextIOWrapper(out_file, write_through=True)
            print(
                (
                    "-------------Calculating Solution (this may take a while)\n"
            ...
            print("- Map {} x {} m".format(scen.size_config.size_x, scen.size_config.size_y))
            print("- Sector has a side of {} m".format(scen.size_config.size_sector))
            ...
            ues_coords = get_ues_time(
                ues_list=scen.getUEsList(), xml_filename=xml_filename
            )
            ...
            plt.xticks(np.arange(-0.5, ncols, 1))
            plt.yticks(np.arange(-0.5, nrows, 1))
    ...
    def _resolve_first_antenna(
        self, antennas_map_m: list, first_antenna_region, chosen_seed: int
    ):
        while True:
            if (
                first_antenna_region is not None
                and antennas_map_m[first_antenna_region] == 1
            ):
                return first_antenna_region
            first_antenna_region = genf.gen_first_antenna_region(
                chosen_seed=chosen_seed, n_sectors=self.scen.n_sectors
            )
    ...
    def _invoke_solver(
        ...
        min_sinr = self.min_sinr
        min_dis = self.min_dis
        if mode == SolutionType.ILP_VARYING:
            solver_varying(
                Max_Space=scen.n_sectors,
                Max_Time=num_slices,
                users_t_m=users_t_m,
        ...
        elif mode == SolutionType.ILP_FIXED:
            solver_fixed(
                Max_Space=scen.n_sectors,
                Max_Time=num_slices,
                users_t_m=users_t_m,
        ...
        elif mode == SolutionType.ILP_SINGLE:
            valid_time = 0
            ...
            ga.ga_solver(
                ...
                snr_map_mn=sinr_map,
                min_sinr_w=db_to_linear(min_sinr),
                distance_mn=distance_mn,
                min_dis=min_dis,
                result_dir=result_dir,
                first_antenna_region=first_antenna_region,
                fitness_func=ga.fitness_pygad,
            )
        elif mode == SolutionType.PGWO:
            gwo.pgwo_solver(
                scenario=scen,
                num_regions=scen.n_sectors,
                num_slices=num_slices,
...
def main():
    ...
    xml_filename = genf.gen_movement_filename(
        "ilp_move_users", chosen_seed, snapshot=True
    )
    min_sinr = 5
    mode = "single"
    min_dis = 2000
    ...
    users_t_m = genf.gen_users_t_m(
        chosen_seed, lambda_poisson=30, num_slices=num_slices
    )
...
def run_all_solvers(
    ...
    for varying in var:
        ...
        for min_snr in min_sinrs:
            ...
            processes.append(Process(target=gen_solution, kwargs=kwargs))
            processes[-1].start()
