...
def main():
  ...
  with open(filename, 'wt') as f:
    ...
    hp.writeScenario(f, object_name= 'eNB', scenario= 'URBAN_MACROCELL', for5g= True)
    hp.writeComment(f, text= "Microcell")
    hp.writeScenario(f, object_name= 'microCell', scenario= 'URBAN_MICROCELL', for5g= True)
    hp.writeComment(f, text= "UEs")
    hp.writeScenarioPerso(f, num_and_scen=[(num_ues_macro, 'URBAN_MACROCELL'), (num_ues_micro, 'URBAN_MICROCELL')], for5g= True)
    hp.writeSeparation(f, "Mobility")
    hp.writeComment(f, text= "eNodeB")
