...
def main():
  ...
  with open(filename, 'wt') as f:
    ...
    hp.writeNumApps(f, numUEs= scen.n_ues, directions= directions, num_multi= num_macros, multi= True)
    hp.writeComment(f, text= "VoIP UL")
    hp.writeMultiAppVoipUL(f, numUEs= scen.n_ues, num_multi= num_macros, number_app= 0)
    hp.writeComment(f, text= "VoIP DL")
    hp.writeMultiAppVoipDL(f, numUEs= scen.n_ues, num_multi= num_macros, number_app= 1, num_apps = 2)
...
def getMicroPositions(macrocells):
  positions = []
  for m in macrocells:
    positions += m.getSmallcellsPositionList()
  return positions
