...
from app.scenarios.five_g.ILP_configs import ilp_sliced_ini, ilp_sliced_ini_per_slice, ilp_ned
from app.run_simulations import run_make, run_simulation_all_slices, run_simulation_per_slice
...
class SimulationPipeline:
    ...
    def __init__(self, chosen_seeds: List[int], size_x: int = 4000, size_y: int = 4000, size_sector: int = 400,
                 n_macros: int = 1, min_sinrs: List[int] = None, modes: List[str] = None, micro_power: int = 30,
                 result_dir: str = "Solutions", project_dir: str = '../Network_CCOpMv', sim_dir: str = '_5G/simulations',
                 csv_dir: str = '_5G/results', extra_dir: List[str] = None, lambda_poisson_gen_users_t_m: int = 30,
                 num_slices: int = 12, simtime_move: int = 1200, per_slice: bool = True, move_config_name: str = 'ilp_move_users',
                 min_dis: int = 2000, first_antenna_region: int = None, min_time: int = 2, disaster_percentage: int = 0,
                 net_dir: str = '_5G/networks', num_bands: List[int] = None, repetitions: int = 1, slice_time: int = 1,
                 p_size: int = 1428, app: str = "video", extra_config_name: str = "video", target_f: float = 10,
                 cmdenv_config: bool = True, interference: bool = False, allrun_solver: bool = False, only_solver: bool = False):
        self.chosen_seeds = chosen_seeds
        self.size_x = size_x
        self.size_y = size_y
        self.size_sector = size_sector
        self.n_macros = n_macros
        self.min_sinrs = min_sinrs or [5, 10, 15]
        self.modes = modes or ['single', 'fixed', 'pgwo2', 'ga']
        self.micro_power = micro_power
        self.result_dir = result_dir
        self.project_dir = project_dir
        self.sim_dir = sim_dir
        self.csv_dir = csv_dir
        self.extra_dir = extra_dir or ['disaster_percentage', 'micro_power', 'chosen_seed']
        self.lambda_poisson_gen_users_t_m = lambda_poisson_gen_users_t_m
        self.num_slices = num_slices
        self.simtime_move = simtime_move
        self.per_slice = per_slice
        self.move_config_name = move_config_name
        self.min_dis = min_dis
        self.first_antenna_region = first_antenna_region
        self.min_time = min_time
        self.disaster_percentage = disaster_percentage
        self.net_dir = net_dir
        self.num_bands = num_bands or [100]
        self.repetitions = repetitions
        self.slice_time = slice_time
        self.p_size = p_size
        self.app = app
        self.extra_config_name = extra_config_name
        self.target_f = target_f
        self.cmdenv_config = cmdenv_config
        self.interference = interference
        self.allrun_solver = allrun_solver
        self.only_solver = only_solver

    def run(self):
        """Run the full pipeline for all configured seeds and modes."""
        if self.only_solver and self.allrun_solver:
            print("The option only_solver cannot be True if allrun_solver is also True.")
            return
        result = run_multiple_seeds(
            chosen_seeds=self.chosen_seeds, size_x=self.size_x, size_y=self.size_y, size_sector=self.size_sector,
            n_macros=self.n_macros, min_sinrs=self.min_sinrs, project_dir=self.project_dir, sim_dir=self.sim_dir,
            csv_dir=self.csv_dir, modes=self.modes, result_dir=self.result_dir, move_config_name=self.move_config_name,
            min_dis=self.min_dis, first_antenna_region=self.first_antenna_region, net_dir=self.net_dir,
            num_bands=self.num_bands, repetitions=self.repetitions, slice_time=self.slice_time, p_size=self.p_size,
            app=self.app, target_f=self.target_f, extra_config_name=self.extra_config_name, cmdenv_config=self.cmdenv_config,
            min_time=self.min_time, micro_power=self.micro_power, extra_dir=self.extra_dir, num_slices=self.num_slices,
            simtime_move=self.simtime_move, per_slice=self.per_slice, allrun_solver=self.allrun_solver,
            disaster_percentage=self.disaster_percentage, only_solver=self.only_solver,
            lambda_poisson_gen_users_t_m=self.lambda_poisson_gen_users_t_m, interference=self.interference
        )
        if result == SUCCESS:
...
def main():
    ...
    pipeline = SimulationPipeline(
        ...
        size_x=4000, size_y=4000, size_sector=400, n_macros=1,
        min_sinrs=[5, 10, 15], modes=['single', 'fixed', 'pgwo2', 'ga'],
        micro_power=30, result_dir="Solutions", project_dir='../Network_CCOpMv',
        sim_dir='_5G/simulations', csv_dir='_5G/results',
        extra_dir=['disaster_percentage', 'micro_power', 'chosen_seed'],
        lambda_poisson_gen_users_t_m=30, num_slices=12, simtime_move=1200, per_slice=True,
        move_config_name='ilp_move_users', min_dis=2000, first_antenna_region=None, min_time=2, disaster_percentage=0,
        net_dir='_5G/networks', num_bands=[100], repetitions=1, slice_time=1, p_size=1428,
        app="video", extra_config_name="video", target_f=10, cmdenv_config=True, interference=False,
        allrun_solver=False, only_solver=False
    )
    pipeline.run()
