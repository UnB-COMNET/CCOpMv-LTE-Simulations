...
def ilp_sliced_ini2(filename, seed, size_y:int =8000, size_x:int =8000, size_sector:int =800, n_macros: int = 2, min_sinr: float = 10, repetitions: int = 5,
                  num_bands: List[int] = [100], multi_carriers: bool = True, slice_time:int = 1, is_micro: bool = True, p_size: int = 40, app: str= "voip", target_f:int = 10,
                  extra_config_name: str = '', result_dir: str = '.', varying: bool = False, network_name: str = '', net_dir: str= '_5G/networks',
                  cmdenv_config: bool = False, micro_power: int = 30, xml_filename: str= 'ilp_fixed_users-sched=MAXCI--0.sna'):
  """This function generates a .ini file to create a simulation with multiple UEs and eNBs using slices of time.
  ...
  optimized_byslice, antennas_regions_byslice, num_enbs_time = parse_results2(gen_solver_result_filename(result_dir, mode, min_sinr), num_slices)
  ...
  #enbs_coords = scen.getAntennasPositionList()    # !!!MUDAR PARA UMA LISTA

  num_ues = len(scen.getUEsList())
  #num_enbs = len(enbs_coords)                     # !!!MUDAR PARA UMA LISTA
...
def parse_results2(filename: str, max_time: int):
  """This function parses the UEs and eNBs necessary information from the solver (ccop_mv_MILP) resulted solution.
